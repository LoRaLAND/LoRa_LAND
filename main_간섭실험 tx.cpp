...
int main(void) {
	...
	dest_ID = 34;//src와 dest ID 자동 설정 (scanf 입력 X)<---------------------------------------------------------------------------------------------------------------------------------------------------------
	//pc.printf(":: ID for the destination : %d\n",dest_ID);
	//pc.scanf("%d", &dest_ID);
	endNode_ID = 12;					//src id
	...
	while (1)
	{
		...
		if (arqEvent_checkEventFlag(arqEvent_dataToSend)) //no ACK, data to send
		{
			...
			arqMain_sendData(arqPdu, pduSize, dest_ID);	
						...
						//무한 전송하는 간섭자의 경우 delay를 최솟값~최솟값*2 사이의 랜덤 값으로 설정
			wait_ms(57);//딜레이 설정(sf 별로 다르게 설정해야함- sf7: 56.6 ms, sf8: 102.9 ms, sf9: 205.8 ms, sf10: 370.7 ms, sf11: 741.4 ms, sf12: 1482.8 ms(overhead size 12byte, payload size 10byte 기준? 계산해주는 사이트를 참고함))
